/* Static validation for the primary-weapon SEO cluster. */
const fs = require('fs');
...
const routes = [
  ...
  '/collectibles/weapons/the-iconoclast/',
  '/collectibles/weapons/axe-dagger/',
  '/collectibles/weapons/veterans-battle-axe/',
  '/collectibles/weapons/great-martyrs-blade/',
  '/collectibles/weapons/obsidian-hammer/',
  '/collectibles/weapons/axatana/',
  '/collectibles/weapons/black-needle/',
  '/collectibles/weapons/clockwork-scythe/',
  '/collectibles/weapons/best-early-weapons/',
];
...
for (const route of routes) {
  ...
  const title = htmlDecode(rawTitle);
  ...
  if (!sitemap.includes(`<loc>${expected}</loc>`)) errors.push(`${route}: sitemap entry missing`);
...
console.log(`Validated ${routes.length} weapon pages: metadata, canonicals, JSON-LD, sitemap entries, images and internal links.`);
